...
describe('owner-only admin security', () => {
  ...
  it('keeps single-owner account deletion allow-listed and removes team mutations', () => {
    ...
    expect(actionSource).toContain("await tx.workspace.delete")
    expect(actionSource).toContain('workspaceDataDeleted: true')
    expect(actionSource).toContain('deletedUserId: user.id')
    expect(actionSource).not.toContain('deleted: user')
    expect(actionSource).not.toContain('workspaceDataPreserved')
