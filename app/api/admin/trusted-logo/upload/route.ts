...
const MAX_BYTES = 4 * 1024 * 1024 // 4 MB — brand logos
...
export async function POST(req: Request) {
	...
	// Same layout as the showcase/blog uploads: public/uploads/<section>/ served
	// via /api/uploads/<section>/<file> with immutable cache (UUID filenames).
	const dir = join(process.cwd(), 'public', 'uploads', 'trusted-logo')
	await mkdir(dir, { recursive: true })
	await writeFile(join(dir, name), buf)

	return NextResponse.json({ url: `/api/uploads/trusted-logo/${name}` })
}
