...
export async function generateMetadata({ params }: Props) {
  ...
  const description = post.seoDescription ?? deriveSeoDescription(post.excerpt, post.content)
...
export default async function PublicBlogPostPage({ params }: Props) {
  ...
  const breadcrumbLd = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: locale === 'fa' ? 'خانه' : 'Home', item: process.env.NEXT_PUBLIC_APP_URL ?? '/' },
      { '@type': 'ListItem', position: 2, name: locale === 'fa' ? 'بلاگ' : 'Blog', item: `${process.env.NEXT_PUBLIC_APP_URL ?? ''}/blog` },
      { '@type': 'ListItem', position: 3, name: post.title, item: `${process.env.NEXT_PUBLIC_APP_URL ?? ''}/blog/${post.slug}` },
    ],
  }
