...
export function FlowEditor({
	...
	function loadTemplate(id: string) {
		...
		if (!tpl) return
		// Deep clone so subsequent edits don't mutate the template.
					...
					<div className="grid gap-3 sm:grid-cols-3">
						{FLOW_TEMPLATES.map((tpl) => (
							<button
								...
								className="group flex flex-col gap-1.5 rounded-xl border border-[var(--border-default)] bg-[var(--bg-base)] p-3 text-start transition-colors hover:border-[var(--border-hover)]"
							>
								<div className="flex items-center gap-2">
								...
								<p className="text-[11px] leading-snug text-[var(--text-muted)]">
									{isFa ? tpl.descFa : tpl.descEn}
								</p>
								<span className="mt-1 text-[10px] text-[var(--text-secondary)] group-hover:text-[var(--text-primary)]">
									{t('useTemplate')} →
								</span>
							</button>
						))}
					</div>
					<p className="mt-3 rounded-lg bg-[var(--bg-base)] px-3 py-2 text-[11px] text-[var(--text-muted)]">
						{t('templatesNote')}
					</p>
				</div>
			)}
