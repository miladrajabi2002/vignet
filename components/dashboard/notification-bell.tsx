	...
	return (
		<div className="relative">
			...
			{mounted && open && createPortal(<>
				<button type="button" aria-label={fa ? 'بستن اعلان‌ها' : 'Close notifications'} onClick={() => setOpen(false)} className="fixed inset-0 z-[98] bg-black/55 sm:bg-black/15" />
				<section ref={panelRef} role="dialog" aria-modal="true" aria-label={t('title')} className="material-select-menu fixed inset-x-3 z-[99] max-h-[calc(100dvh-5rem)] overflow-hidden rounded-[1.65rem] border border-black/15 bg-white shadow-[0_28px_90px_rgba(0,0,0,0.34)] [bottom:max(env(safe-area-inset-bottom),0.75rem)] [animation:spatial-pop_220ms_cubic-bezier(.2,.8,.2,1)] motion-reduce:animate-none sm:inset-x-auto sm:bottom-auto sm:end-5 sm:top-[5.75rem] sm:w-96 xl:top-[6.75rem]">
						...
						{items.length === 0 ? (
							...
							<ul className="space-y-1">
								{items.map((item) => {
									...
									const content = <div className={cn('flex min-h-16 gap-3 rounded-xl px-3 py-2.5 transition-colors hover:bg-black/[0.05]', !item.read && 'bg-amber-50')}><span className={cn('mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-xl', !item.read ? 'bg-black text-white' : 'bg-zinc-100 text-zinc-600')}><Icon className="h-3.5 w-3.5" /></span><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="truncate text-xs font-bold text-black/85">{item.title}</p>{!item.read && <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-amber-500" />}</div>{item.body && <p className="mt-1 line-clamp-2 text-[11px] leading-5 text-black/60">{item.body}</p>}<time className="mt-1 block text-[11px] text-black/45">{formatDateTime(new Date(item.createdAt), fa ? 'fa' : 'en')}</time></div></div>
									return <li key={item.id}>{item.link ? <Link href={item.link} onClick={() => { void markRead(item.id); setOpen(false) }} className="block rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-1">{content}</Link> : <button type="button" onClick={() => void markRead(item.id)} className="w-full rounded-xl text-start focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-1">{content}</button>}</li>
								})}
							</ul>
						)}
					</div>
