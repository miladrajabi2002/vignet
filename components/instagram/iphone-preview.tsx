...
function PhoneFrame({ children }: { children: React.ReactNode }) {
        return (
                <div
                        className="relative mx-auto aspect-[9/19.5] w-full max-w-[260px] rounded-[3rem] p-[8px] shadow-[0_24px_50px_-12px_rgba(0,0,0,0.4),0_0_0_1px_rgba(255,255,255,0.06)_inset,0_0_0_2px_rgba(0,0,0,0.4)]"
                        style={{
                                background:
                                        'linear-gradient(135deg, #2a2a2c 0%, #1c1c1e 25%, #3a3a3c 50%, #1c1c1e 75%, #2a2a2c 100%)',
                        }}
                        aria-hidden
                >
                        {/* Titanium inner rim */}
                        <div
                                className="pointer-events-none absolute inset-[3px] rounded-[2.8rem] ring-1 ring-white/5"
                                aria-hidden
                        />

                        {/* Side buttons — Action button + volume up/down + power */}
                        <div className="absolute -start-[2px] top-[15%] h-2 w-[3px] rounded-l bg-[#1c1c1e]" />
                        <div className="absolute -start-[2px] top-[20%] h-7 w-[3px] rounded-l bg-[#1c1c1e]" />
                        <div className="absolute -start-[2px] top-[27%] h-7 w-[3px] rounded-l bg-[#1c1c1e]" />
                        <div className="absolute -start-[2px] top-[34%] h-7 w-[3px] rounded-l bg-[#1c1c1e]" />
                        <div className="absolute -end-[2px] top-[28%] h-14 w-[3px] rounded-r bg-[#1c1c1e]" />

                        {/* Screen */}
                        <div className="relative h-full w-full overflow-hidden rounded-[2.4rem] bg-white">
                                {/* Dynamic Island — plain black pill (no status icons inside) */}
                                <div className="absolute left-1/2 top-[8px] z-30 h-[24px] w-[80px] -translate-x-1/2 rounded-full bg-black shadow-[0_0_0_1px_rgba(255,255,255,0.04)]">
                                        {/* Tiny camera dot */}
                                        <div className="absolute end-[8px] top-1/2 h-[7px] w-[7px] -translate-y-1/2 rounded-full bg-[#0a0a0a] ring-1 ring-white/10" />
                                </div>
                                {children}
                        </div>
                </div>
...
// Forced LTR so 9:41 is always on the LEFT and signal/wifi/battery on the
// RIGHT — exactly like a real iPhone, regardless of the app's RTL setting.
// The Dynamic Island sits between them (centered), uninvolved.

function StatusBar({ dark = false }: { dark?: boolean }) {
        return (
                <div
                        dir="ltr"
                        className="relative z-20 flex h-[38px] items-center justify-between px-5 pt-2 text-[11px] font-semibold"
                        style={{
                                fontFamily:
                ...
                >
                        ...
                        <div className="flex items-center gap-[4px]">
                                ...
                                <svg width="14" height="10" viewBox="0 0 16 12" fill="currentColor" aria-hidden>
                                        <path d="M8 11.2l1.7-2.1a2.2 2.2 0 00-3.4 0L8 11.2z" />
                                        <path
                                                d="M3.2 5.5a8.2 8.2 0 019.6 0"
                                                stroke="currentColor"
                                                strokeWidth="1.6"
                                                fill="none"
                                                strokeLinecap="round"
                                ...
                                <div className="flex items-center gap-[1px]">
                                        <div
                                                className={`relative h-[10px] w-[22px] rounded-[3px] border ${dark ? 'border-white/50' : 'border-black/40'}`}
                                                style={{ padding: '1.5px' }}
                                        >
                                                <div
                                        ...
                                        <div
                                                className={`h-[3.5px] w-[1.5px] rounded-e ${dark ? 'bg-white/50' : 'bg-black/40'}`}
                                        />
                                </div>
                        </div>
                </div>
...
function DMScreen(props: ScreenProps) {
        ...
        return (
                ...
                >
                        ...
                        {/* Chat header — IG-style, forced LTR so the back arrow
                            sits on the LEFT (per user request) and phone/video
                            icons sit on the RIGHT. */}
                        <div dir="ltr" className="flex items-center gap-2.5 border-b border-black/[0.06] px-3 py-2">
                                <ArrowRight className="h-5 w-5 shrink-0 text-black" strokeWidth={2.5} aria-label="بازگشت" />
                                <Avatar url={accountAvatarUrl} name={accountUsername} size={32} ring />
                        ...
                        {/* Messages — scrollable, all messages render */}
                        <div className="flex-1 space-y-1.5 overflow-y-auto px-3 py-3 no-scrollbar">
                                ...
                                <div className="flex flex-col items-center pt-1 pb-3">
                                        ...
                                        <p className="mt-2 text-[12px] font-bold text-black">{accountUsername}</p>
                        ...
                        {/* Input bar — forced LTR: camera on LEFT, send on RIGHT */}
                        <div dir="ltr" className="flex items-center gap-2 border-t border-black/[0.06] px-3 py-2.5 pb-4">
                                <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-black/15 text-black">
                                        <Camera className="h-[14px] w-[14px]" />
                                </div>
                                <div className="flex flex-1 items-center rounded-full border border-black/15 px-3.5 py-2">
                                        <span className="text-[12px] text-black/40">پیام…</span>
                                </div>
                                <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-black/15 text-black">
                                        <Mic className="h-[14px] w-[14px]" />
                                </div>
                                <Send className="h-[18px] w-[18px] shrink-0" style={{ color: '#dd2a7b' }} strokeWidth={2} />
                        </div>
...
function MessageBubble({ message }: { message: AutomationMessage }) {
        ...
        // QUICK_REPLY — text + tappable button pills below
                                ...
                                {buttons.length > 0 && (
                                        <div className="mt-2 flex flex-col gap-1.5">
                                                {buttons.slice(0, 3).map((b, i) => (
                                                        <button
                                                                key={i}
                                                                className="rounded-lg border border-white/40 bg-white/15 px-2.5 py-1.5 text-[11px] text-white backdrop-blur transition-colors hover:bg-white/25"
                                                        >
                                                                {b}
                                                        </button>
                                                ))}
...
// ── Story screen — DM view with "replied to story" banner ─────────────────
// Instead of showing the story viewer itself, this now mirrors Instagram's
// actual DM thread when someone replies to a story: a small story thumbnail
// + "replied to their story" context bar, the user's reply as an incoming DM
// bubble, and the bot's auto-response below. Same header/input bar as DM.
...
function StoryScreen(props: ScreenProps) {
        ...
        return (
                ...
                >
                        ...
                        {/* Chat header — same as DM, back arrow on LEFT */}
                        <div dir="ltr" className="flex items-center gap-2.5 border-b border-black/[0.06] px-3 py-2">
                                <ArrowRight className="h-5 w-5 shrink-0 text-black" strokeWidth={2.5} aria-label="بازگشت" />
                                <Avatar url={accountAvatarUrl} name={accountUsername} size={32} ring />
                                <div className="min-w-0 flex-1">
                        ...
                        <div className="flex-1 space-y-1.5 overflow-y-auto px-3 py-3 no-scrollbar">
                                ...
                                {/* "Replied to story" context bar — IG-style */}
                                <div className="flex items-center gap-2.5 rounded-2xl bg-black/[0.04] px-2.5 py-2">
                                        <div
                                        ...
                                        >
                                                <div className="absolute inset-0 flex items-center justify-center">
                                                        <span className="text-[8px] font-semibold text-white/90">استوری</span>
                                                </div>
                                        </div>
                                        <p className="text-[10px] leading-tight text-black/60">
                                                <span className="font-semibold text-black/80">{accountUsername}</span>{' '}
                                                به استوری شما پاسخ داد
                                        </p>
                                </div>

                                {/* User's reply (the trigger keyword) — incoming DM bubble */}
                                {userText.trim() ? (
                                        <Bubble side="user">{userText}</Bubble>
                                ) : (
                                        <Bubble side="user" muted>
