...
function PhoneFrame({ children }: { children: React.ReactNode }) {
	return (
		<div
			className="relative mx-auto aspect-[9/19.5] w-full max-w-[300px] rounded-[3.2rem] p-[10px] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.4),0_0_0_1px_rgba(255,255,255,0.06)_inset,0_0_0_2px_rgba(0,0,0,0.4)]"
			style={{
				background:
		...
		>
			...
			<div className="absolute -start-[2px] top-[20%] h-8 w-[3px] rounded-l bg-[#1c1c1e]" />
			<div className="absolute -start-[2px] top-[27%] h-8 w-[3px] rounded-l bg-[#1c1c1e]" />
			<div className="absolute -start-[2px] top-[34%] h-8 w-[3px] rounded-l bg-[#1c1c1e]" />
			<div className="absolute -end-[2px] top-[28%] h-16 w-[3px] rounded-r bg-[#1c1c1e]" />
			...
			<div className="relative h-full w-full overflow-hidden rounded-[2.6rem] bg-white">
				...
				<div className="absolute left-1/2 top-[10px] z-30 h-[26px] w-[88px] -translate-x-1/2 rounded-full bg-black shadow-[0_0_0_1px_rgba(255,255,255,0.04)]">
					{/* Tiny camera dot */}
					<div className="absolute end-[10px] top-1/2 h-[8px] w-[8px] -translate-y-1/2 rounded-full bg-[#0a0a0a] ring-1 ring-white/10" />
				</div>
				{children}
			</div>
		</div>
...
function StatusBar({ dark = false }: { dark?: boolean }) {
	...
	return (
		<div
			className={`relative z-20 flex h-[42px] items-center justify-between px-7 pt-2.5 text-[12px] font-semibold ${fg}`}
		...
		>
			...
			<div className="flex items-center gap-[5px]">
				...
				<div className="flex items-center gap-[1px]">
					<div
						className={`relative h-[11px] w-[24px] rounded-[3.5px] border ${dark ? 'border-white/50' : 'border-black/40'}`}
						style={{ padding: '1.5px' }}
					>
						<div
					...
					<div
						className={`h-[4px] w-[1.5px] rounded-e ${dark ? 'bg-white/50' : 'bg-black/40'}`}
					/>
				</div>
			</div>
		</div>
	...
	return (
		...
		>
			...
			<div className="flex items-center gap-2.5 border-b border-black/[0.06] px-3 py-2">
				<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-black" aria-hidden>
			...
			<div className="flex items-center gap-2 border-t border-black/[0.06] px-3 py-2.5 pb-4">
				<div className="flex h-7 w-7 items-center justify-center rounded-full border border-black/15 text-black">
				...
				<Send className="h-[18px] w-[18px]" style={{ color: '#dd2a7b' }} strokeWidth={2} />
			</div>
...
function StoryScreen(props: ScreenProps) {
	...
	return (
		...
		>
			...
			{/* Story body — vignette so text stays readable */}
			<div className="relative z-10 flex h-[calc(100%-200px)] items-center justify-center px-8">
				<p className="text-center text-[14px] font-medium leading-relaxed text-white/95">
					{userText.trim() ? `"${userText}"` : 'پاسخ استوری نمونه…'}
				</p>
			</div>

			{/* Reply bar + bot's auto-response overlay */}
			<div className="absolute inset-x-0 bottom-0 z-20 space-y-2 bg-gradient-to-t from-black/70 via-black/40 to-transparent p-3 pt-8 pb-5">
				{/* Bot's automatic reply — only when STATIC/MULTI_MESSAGE */}
				{(replyMode === 'STATIC' || replyMode === 'MULTI_MESSAGE') && visibleMessages.length > 0 && (
					<div className="space-y-1.5">
						{visibleMessages.slice(0, 2).map((m) => (
							<div key={m.id} className="flex justify-start">
								<div className="max-w-[80%] rounded-2xl rounded-bl-md bg-white px-3 py-1.5 text-[11px] text-black shadow-lg">
				...
				{(replyMode === 'STATIC' || replyMode === 'MULTI_MESSAGE') && reply && visibleMessages.length === 0 && (
				...
				{replyMode === 'AI' && (
					<div className="flex justify-start">
						<div className="rounded-2xl rounded-bl-md bg-white px-3 py-2 text-[11px] text-black shadow-lg">
							<TypingDots />
						</div>
					</div>
				)}
				...
				{followUpEnabled && followUpMessage?.trim() && (
					<div className="text-center text-[9px] text-white/60">
						پیگیری بعد از {(followUpDelayMin ?? 60).toLocaleString('fa-IR')} دقیقه
					</div>
				)}

				{/* Reply input row */}
				<div className="flex items-center gap-2">
					<div className="flex flex-1 items-center rounded-full border border-white/40 bg-black/25 px-4 py-2.5 backdrop-blur-md">
						<span className="text-[12px] text-white/80">
							{userText.trim() || 'Send Message'}
						</span>
					</div>
			...
			{/* Home indicator */}
			<div className="absolute bottom-[6px] left-1/2 z-30 h-[5px] w-[110px] -translate-x-1/2 rounded-full bg-white/60" />
		</div>
	)
}
