...
export function ProductShowcaseRail({
        ...
        return (
                <section
                        ...
                        dir={isFa ? 'rtl' : 'ltr'}
                        // Shrink-to-fit (unless expanded): a rail with one or two cards must
                        // hug the agent's side of the thread like the bubble above it, not
                        // fill the full row width and cluster the cards on the customer's
                        // side. With ten cards the rail is wider than the column anyway, so
                        // the clamped max-width keeps the scroll behaviour identical. The
                        // expanded grid needs a definite width for auto-fill columns, so it
                        // stays w-full.
                        className={cn('min-w-0 max-w-full', expanded ? 'w-full' : 'w-fit', className)}
                        style={accentStyle}
                >
