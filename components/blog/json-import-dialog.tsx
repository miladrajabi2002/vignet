...
function validateSeo(post: Partial<BlogPostData>): SeoCheck[] {
	...
	if (tl >= 30 && tl <= 60) checks.push({ label: 'عنوان: ۳۰–۶۰ کاراکتر', status: 'pass' })
		...
		checks.push({ label: 'عنوان: ۳۰–۶۰ کاراکتر', status: 'warn', hint: `${tl} کاراکتر` })
	...
	if (sd >= 70 && sd <= 160)
		...
		checks.push({ label: 'متا توضیحات (۷۰–۱۶۰)', status: 'warn', hint: `${sd} کاراکتر` })
...
export function JsonImportDialog({ open, onClose, onImport }: JsonImportDialogProps) {
	...
	// The generated Vigent-branded poster prompt (built from the parsed post).
	...
	function handleParse() {
		...
		try {
			...
			if (Array.isArray(obj.seoKeywords))
				out.seoKeywords = obj.seoKeywords.filter(
	...
	/**
	 * Build a Vigent-branded poster prompt from the parsed post. Uses the post
	 * title as the topic + the first 1-2 sentences of content as the summary.
	 * The prompt enforces: 1536×1024, bilingual "ویجنت / VIGENT" wordmark,
	 * "vigent.ir" URL, cinematic editorial style, accent color by topic keyword.
	 */
	function buildPosterPrompt() {
		...
		// Pick an accent color based on topic keywords.
		const text = (topic + ' ' + (parsed.content || '')).toLowerCase()
		let accent = 'teal #06b6d4'
		let accentLabel = 'teal'
		if (/فروش|سفارش|درآمد|مشتری|woocommerce|ووکامرس|خرید/.test(text)) {
			accent = 'warm amber #f59e0b'
			accentLabel = 'amber'
		} else if (/پشتیبانی|رضایت|کمک|support|success/.test(text)) {
			accent = 'emerald #10b981'
			accentLabel = 'emerald'
		} else if (/ai|هوش مصنوعی|ایجنت|agent|آینده|نوآور/.test(text)) {
			accent = 'violet #8b5cf6'
			accentLabel = 'violet'
		}

		const prompt = `A cinematic editorial blog cover poster for Vigent, exactly 1536x1024 pixels landscape (3:2 ratio).
...
- Monochrome base (charcoal #0a0a0a → soft white #f5f5f5) with ONE accent color: ${accent} for the topic visual.
...
NEGATIVE: clutter, cartoon, anime, photorealistic humans, busy background, neon overload, drop shadows on text, comic-sans, multiple focal points, text-heavy, low-contrast text, 3D plastic render, watermarks, signatures.

The accent color is ${accentLabel}. Build the poster now.`
			...
			>
				...
				<div className="max-h-[calc(100vh-10rem)] overflow-y-auto p-5 space-y-4">
					...
					<p className="text-xs leading-relaxed text-zinc-500">
						...
						استخراج می‌کند و <strong className="text-zinc-300">slug</strong> را به انگلیسی
						امن تبدیل می‌کند. سپس با یک کلیک همه فیلدهای ادیتور پر می‌شوند.
					</p>
						...
						<div className="space-y-4 border-t border-zinc-800 pt-4">
							...
							<div className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-3 space-y-3">
								...
								<p className="text-[11px] leading-relaxed text-zinc-500">
									پرامپت آماده برای Grok Image / Midjourney / DALL-E — شامل ابعاد
									۱۵۳۶×۱۰۲۴، اسم دوزبانه «ویجنت / VIGENT»، آدرس vigent.ir، و رنگ accent
									خودکار بر اساس موضوع.
								</p>
								...
								{imagePrompt && (
									<details className="rounded-md border border-zinc-800 bg-zinc-950/50 p-2">
										...
										<p
											dir="ltr"
											className="mt-2 whitespace-pre-wrap break-words font-mono text-[10px] text-zinc-500 ltr text-left"
										>
											{imagePrompt}
										</p>
									</details>
									...
									<div className="space-y-2">
										<div className="flex items-center justify-between gap-2">
											<span className="text-[11px] text-zinc-400">
												پرامپت پوستر آماده — {posterPrompt.length} کاراکتر
											</span>
											<button
												onClick={copyPosterPrompt}
										...
										<pre
											dir="ltr"
											className="max-h-64 overflow-y-auto whitespace-pre-wrap break-words rounded-md border border-zinc-800 bg-zinc-950/50 p-2.5 font-mono text-[10px] leading-relaxed text-zinc-400 ltr text-left"
										>
											{posterPrompt}
										</pre>
									</div>
