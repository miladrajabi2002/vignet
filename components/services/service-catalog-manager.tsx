    ...
    <section className="space-y-4">
      ...
      {open && (
        <div className="spatial-surface rounded-[1.5rem] p-4 sm:p-5">
          <div className="mb-4 flex items-center justify-between gap-3">
            ...
            <button type="button" onClick={() => setOpen(false)} className="grid h-9 w-9 place-items-center rounded-xl border border-[var(--border-default)] text-[var(--text-muted)] hover:text-[var(--text-primary)]" aria-label="بستن">
              <X className="h-4 w-4" />
            </button>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="block">
              ...
              <input className="input" value={form.name} onChange={(event) => setForm((value) => ({ ...value, name: event.target.value }))} placeholder="مثلاً مشاوره اولیه" />
            </label>
            <label className="block">
          ...
          <div className="mt-4 flex justify-end">
            <button type="button" disabled={saving || form.name.trim().length < 2} onClick={createService} className="spatial-press inline-flex min-h-11 items-center gap-2 rounded-xl bg-black px-5 text-xs font-bold text-white disabled:opacity-40">
      ...
      {items.length ? (
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {items.map((item) => (
            <article key={item.id} className={cn('spatial-surface rounded-[1.5rem] p-4 transition-opacity', !item.active && 'opacity-55')}>
              <div className="flex items-start justify-between gap-3">
                ...
                <button type="button" disabled={busyId === item.id} onClick={() => toggle(item)} aria-label={item.active ? 'غیرفعال کردن' : 'فعال کردن'} className={cn('grid h-10 w-10 place-items-center rounded-xl border', item.active ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-black/10 text-black/35')}>
                  {busyId === item.id ? <Loader2 className="h-4 w-4 animate-spin"/> : <Power className="h-4 w-4"/>}
                </button>
              </div>
              ...
              <Link href="/appointments" className="mt-4 inline-flex min-h-10 w-full items-center justify-center rounded-xl border border-black/10 text-[10px] font-bold transition-colors hover:bg-black hover:text-white">تنظیم برنامه و رزرو</Link>
            </article>
          ))}
        </div>
      ) : (
        <div className="rounded-[1.5rem] border border-dashed border-black/15 bg-white p-10 text-center">
          ...
          <p className="mt-1 text-xs text-black/45">اولین خدمت را اضافه کنید تا در اختیار ایجنت قرار بگیرد.</p>
        </div>
      )}
    </section>
  )
