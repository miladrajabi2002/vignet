      ...
      {services.some((service) => service.active) ? (
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(22rem,0.65fr)]">
          <section className="min-w-0 rounded-3xl border border-[var(--border-default)] bg-white p-5 shadow-[var(--shadow-soft)] sm:p-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-lg font-semibold text-[var(--text-primary)]">{copy.appointments}</h2><p className="mt-1 text-sm text-[var(--text-secondary)]">{formatDay(selectedDate, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p></div><div className="flex gap-2"><LocalizedDatePicker value={selectedDate} onValueChange={(nextDate) => { setSelectedDate(nextDate); void loadAppointments(nextDate) }} locale={locale} ariaLabel={copy.date} className="min-w-56" /><button type="button" onClick={() => loadAppointments(selectedDate)} aria-label={copy.refresh} className="grid h-11 w-11 place-items-center rounded-xl border border-[var(--border-default)]">{loadingAppointments ? <Loader2 className="h-4 w-4 animate-spin motion-reduce:animate-none" /> : <RefreshCw className="h-4 w-4" />}</button></div></div>
