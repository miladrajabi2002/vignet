...
export function verifyAdminCredentialsDetailed(
	...
	if (!totpOk) {
		// تولید کد فعلی برای کمک به دیباگ (بدون لو رفتن secret)
		const counter = Math.floor(Date.now() / 30_000)
		const currentCode = totpAt(totpConfigured, counter)
		console.error(
			`[ADMIN-AUTH-DEBUG] ❌ FAIL گام ۴ (TOTP_INVALID): کد وارد شده هم‌خوانی ندارد`,
		)
		console.error(
			`[ADMIN-AUTH-DEBUG]    کدی که سرور الان تولید می‌کند: ${currentCode} (با اپ گوشی مقایسه کنید)`,
		)
		console.error(
