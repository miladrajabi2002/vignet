...
import { conversationWhere, json, type Selection } from './types'
...
export async function startImprovement(workspaceId: string, agentId: string, actorId: string, input: Selection, scheduled = false) {
  const run = await prisma.$transaction(async (tx) => {
    ...
    const conversations = await tx.conversation.findMany({
      where: conversationWhere(workspaceId, agentId, input),
...
export async function improvementOverview(workspaceId: string, agentId: string, page = 1, history = false, runPage = 1) {
  const [runs, suggestions, pendingCount, suggestionTotal, runTotal, activeRun] = await Promise.all([
    prisma.improvementRun.findMany({ where: { workspaceId, agentId }, orderBy: { createdAt: 'desc' }, take: 25, skip: (runPage - 1) * 25,
      select: { id: true, status: true, total: true, error: true, createdAt: true, finishedAt: true,
    ...
    prisma.improvementRun.findFirst({ where: { workspaceId, agentId, status: { in: ['QUEUED', 'RUNNING'] } }, select: { id: true, status: true, total: true, error: true, createdAt: true, reviews: { select: { id: true, status: true } } } }),
  ])
  ...
  return { runs, runTotal, activeRun, suggestionTotal, suggestions: suggestions.map((s) => ({ ...s, conversationCount: Number(evidenceCounts.find((e) => e.id === s.id)?.conversations ?? 0), monitoring: monitoring.some((m) => m.suggestionId === s.id) ? { reviewed: Number(monitoring.find((m) => m.suggestionId === s.id)!.reviewed), recurring: Number(monitoring.find((m) => m.suggestionId === s.id)!.recurring) } : null, knowledgeStatus: knowledge.find((k) => s.changes.some((c) => !c.revertedAt && c.targetId === k.id))?.status ?? null, evidence: s.evidence.map((e) => ({ ...e, message: { ...e.message, content: e.message.content.slice(0, 1200) } })) })), pendingCount }
}

export function publicError(error: unknown) {
  const code = error instanceof Error ? error.message : 'FAILED'
  const allowed = ['NOT_FOUND', 'RUN_ACTIVE', 'NO_CONVERSATIONS', 'SELECTION_CHANGED', 'QUEUE_UNAVAILABLE', 'CONFLICT', 'AI_UNAVAILABLE', 'MISSING_INFORMATION', 'INVALID_BEHAVIOR', 'KNOWLEDGE_CHANGED', 'BEHAVIOR_CHANGED', 'UNSUPPORTED_KNOWLEDGE', 'TEST_REQUIRED', 'INGESTION_UNAVAILABLE', 'INVALID_CONTENT', 'NO_TEST_MESSAGE', 'TOOL_MANUAL', 'EVIDENCE_REQUIRED']
