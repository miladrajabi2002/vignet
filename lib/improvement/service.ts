...
export async function improvementOverview(workspaceId: string, agentId: string, page = 1, history = false, runPage = 1) {
  const [runs, suggestions, pendingCount, suggestionTotal, runTotal, activeRun] = await Promise.all([
    ...
    prisma.improvementSuggestion.findMany({ where: { workspaceId, agentId, status: history ? { in: ['APPLIED', 'REVERTED'] } : 'PENDING' }, orderBy: [{ createdAt: 'desc' }, { id: 'desc' }], take: 25, skip: (page - 1) * 25,
    ...
    prisma.improvementSuggestion.count({ where: { workspaceId, agentId, status: history ? { in: ['APPLIED', 'REVERTED'] } : 'PENDING' } }),
