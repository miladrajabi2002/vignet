        ...
        return {
                ...
                async sendText(chatId: string, text: string, opts?: SendOptions): Promise<void> {
                        ...
                        if (!res.ok) {
                                ...
                                let hint = ''
                                if (/"code"\s*:\s*230/.test(detail)) {
                                        hint =
                                                ' [راه‌حل: اپ شما در Development Mode است و فقط Standard Access به instagram_manage_messages دارد. ' +
                                                'برای پاسخ به DM باید App Review بزنید و Advanced Access بگیرید. ' +
                                                'developers.facebook.com → اپ → App Review → Permissions and Features → ' +
                                                'instagram_manage_messages → Request Advanced Access. تا Approval، فقط اکانت‌های tester/admin می‌توانند پیام بفرستند/بگیرند.]'
                                ...
                                } else if (/"code"\s*:\s*613/.test(detail)) {
                                        hint = ' [راه‌حل: اکانت اینستاگرام قابلیت پاسخ‌دهی از طریق API را ندارد — مطمئن شوید Business/Creator است.]'
                                } else if (/"code"\s*:\s*4\b|rate.*limit/i.test(detail)) {
                                        hint = ' [راه‌حل: محدودیت نرخ API — چند ثانیه صبر کنید و دوباره تلاش کنید.]'
                                }
                                // 10 (permission), 200-299 (permission/rate), 613 (capability) — the
                                // most common causes of a "valid token but reply refused" failure.
                                throw new Error(
                                        `INSTAGRAM sendText failed (${res.status}): ${detail}.${hint}`,
                                )
                        }
                },
