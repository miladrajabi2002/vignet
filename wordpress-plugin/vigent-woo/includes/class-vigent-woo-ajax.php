...
class Vigent_Woo_Ajax {
        ...
        public function ajax_sync_batch() {
                ...
                // Bump the PHP memory limit for this request only. Building a batch
                // of 50 order payloads (each with line items, billing, shipping, meta)
                // can spike memory on stores with thousands of orders, and the default
                // WP_MEMORY_LIMIT (often 40M–128M on shared hosts) is too tight. We
                // raise it to 256M if the host allows ini_set — silently ignored if
                // the host disables ini_set (safe on managed WP hosts).
                $current_limit = ini_get( 'memory_limit' );
                if ( $current_limit && function_exists( 'wp_convert_hr_to_bytes' ) ) {
                        $current_bytes = wp_convert_hr_to_bytes( $current_limit );
                        $target_bytes  = 256 * 1024 * 1024; // 256M
                        if ( $current_bytes > 0 && $current_bytes < $target_bytes ) {
                                @ini_set( 'memory_limit', '256M' );
                        }
                }

                // Log the incoming batch request so the debug log shows what we were
                // trying to do when (if) something crashes. Without this, a fatal error
                // leaves no trace of which kind/offset was being processed.
                $this->core()->debug_log( 'ajax_sync_batch ENTER', array(
                        'kind'   => $kind,
                        'offset' => $offset,
                        'memory' => function_exists( 'memory_get_usage' ) ? round( memory_get_usage( true ) / 1024 / 1024, 1 ) . 'MB' : '',
                        'limit'  => ini_get( 'memory_limit' ),
                ) );

                // Wrap the actual sync in a try/catch. Most PHP errors (TypeError,
                // DivisionByZeroError, etc.) are throwable in PHP 8+ and will be caught
                // here. Fatal errors (E_ERROR — memory_limit exhausted, class-not-found)
                // are NOT catchable here, but the register_shutdown_function in
                // vigent-woo.php will log them to the same debug.log file.
                try {
                        $result = Vigent_Woo_Sync::instance()->sync_batch( $kind, $offset, 50, $filter );
                        $this->core()->debug_log( 'ajax_sync_batch EXIT', array(
                                'kind'   => $kind,
                                'offset' => $offset,
                                'sent'   => isset( $result['sent'] ) ? $result['sent'] : 0,
                                'errors' => isset( $result['errors'] ) ? count( $result['errors'] ) : 0,
                                'total'  => isset( $result['total'] ) ? $result['total'] : 0,
                                'done'   => isset( $result['done'] ) ? $result['done'] : false,
                        ) );
                        wp_send_json_success( $result );
                } catch ( \Throwable $t ) {
                        // PHP 7.0+ — Throwable covers both Error and Exception.
                        $this->core()->debug_log( 'ajax_sync_batch EXCEPTION', array(
                                'kind'        => $kind,
                                'offset'      => $offset,
                                ...
                                'file'        => $t->getFile(),
                                'line'        => $t->getLine(),
                                'memory_used' => function_exists( 'memory_get_usage' ) ? round( memory_get_usage( true ) / 1024 / 1024, 1 ) . 'MB' : '',
                        ...
                        wp_send_json_error( array(
                                'message' => sprintf(
                                        ...
                                        __( 'خطای داخلی افزونه (%1$s): %2$s. لاگ دیباگ را ببینید.', 'vigent-woo' ),
                                        get_class( $t ),
                                        $t->getMessage()
                                ),
                        ), 500 );
                } catch ( \Exception $e ) {
                        // PHP 5.x fallback (shouldn't happen on PHP 7+, but defensive).
                        $this->core()->debug_log( 'ajax_sync_batch EXCEPTION', array(
                                'kind'      => $kind,
                                'offset'    => $offset,
                        ...
                        wp_send_json_error( array(
                                'message' => sprintf(
                                        __( 'خطای داخلی افزونه: %s. لاگ دیباگ را ببینید.', 'vigent-woo' ),
                                        $e->getMessage()
                                ),
                        ), 500 );
                }
