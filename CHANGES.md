...
## اصلاح رفتار «فقط اتوماسیون» — ۶ سپتامبر ۲۰۲۶

- پیام اینستاگرام بدون سناریو، پاسخ ثابت فعال یا مرحلهٔ تأیید فالو، پیش از ایجاد مخاطب و گفتگو کنار گذاشته می‌شود؛ بدون پاسخ، فراخوانی ایجنت، ارجاع اپراتور یا اعلان. این قاعده برای گفتگوهای قبلاً ارجاع‌شده نیز برقرار است.
- پیام پیش‌فرض ارجاع اتوماسیون و مسیر `AUTOMATION_ONLY_ESCALATED` حذف شدند. این تغییر جایگزین رفتار A16 برای پیام‌های بدون سناریو است.
- حالت فقط اتوماسیون، تبدیل صوت به متن و پاسخ عمومی مدیا/انتظار/لغو پیام را اجرا نمی‌کند؛ پاسخ‌های صریحاً تنظیم‌شده و سناریوهای منطبق همچنان اجرا می‌شوند.
- رویداد فنی وبهوک برای جلوگیری از پردازش تکراری تکمیل می‌شود؛ پیام نامنطبق در صندوق گفتگو ثبت نمی‌شود.
- تست‌های رگرسیون: دایرکت، کامنت، استوری و واکنش نامنطبق، گفتگوی قبلاً ارجاع‌شده، مدیا، عبارت لغو، سناریوهای منطبق، پاسخ ثابت تنظیم‌شده و سیاست مستقل هر نوع رویداد.


## فعال‌سازی اصلاح اتوماسیون و اصلاح ارجاع‌های قبلی — ۶ سپتامبر ۲۰۲۶

- پردازشگر پیام‌های زنده با نسخهٔ کامیت‌شدهٔ `85313e679` در `/var/www/vigent.ir/releases/automation-only-85313e679` فعال شد. سرویس `vignet-worker` با این مسیر در PM2 ذخیره شده است؛ وبهوک وب همچنان پیام‌ها را به صف پایدار همین پردازشگر می‌فرستد. دیپلوی عادی بعدی می‌تواند مسیر پردازشگر را به checkout اصلی برگرداند.
- دایرکت، کامنت و استوری همگی سیاست مستقل خود را رعایت می‌کنند؛ در حالت فقط اتوماسیون، پیام بدون سناریو یا پاسخ تعریف‌شده وارد صندوق گفتگو نمی‌شود و پاسخ/ارجاع ندارد.
- `scripts/repair-automation-only-handoffs.ts` با dry-run پیش‌فرض و اجرای صریح `--apply` اضافه شد. فقط ارجاع‌های دارای دلیل دقیق خطای اتوماسیون را اصلاح می‌کند؛ گفتگوهای دارای دخالت اپراتور یا ارجاع فعال با دلیل دیگر کنار گذاشته می‌شوند. پیش از تغییر، نسخهٔ پشتیبان خصوصی ذخیره و تغییرات همزمان بررسی می‌شوند.
- ۸۱۱ ارجاع اشتباه بسته شد: ۸۰۷ مورد در حساب سید مهدی حسینی و ۴ مورد در حساب صمدی. ۸۰۶ گفتگوی صرفاً خودکار به RESOLVED رفت و ۵ گفتگوی دارای پاسخ‌های قبلی OPEN شد؛ `handedOff` برای همه غیرفعال شد. ۱۸ اعلان مرتبطِ خوانده‌نشده خوانده شد. هیچ پیام اصلی حذف نشد.
- پشتیبان: `/var/backups/vignet/automation-handoff-repair/repair-2026-09-06T12-34-37-237Z.json`؛ نتیجهٔ اجرا در فایل مجاور `.result.json`.
- اعتبارسنجی: ۴۷ تست مرتبط روی نسخهٔ منتشرشده موفق، TypeScript بدون خطا، اجرای مجدد dry-run برابر صفر، ارجاع اشتباه فعال باقی‌مانده صفر؛ ۱۰ کامنت واقعی پس از انتشار بدون گفتگو/پیام پاسخ تکمیل شدند. سلامت دیتابیس و Redis تأیید شد.
- دستور `graphify update .` اجرا شد اما ابزار graphify روی سرور نصب نیست.

## پنل ادمین — تب‌های پرونده کاربر، حذف ستون کسب‌وکار و تومان‌سازی قیمت‌ها — ۷ سپتامبر ۲۰۲۶

- پرونده کاربر در `/admin/users/[userId]` به ۶ تب تبدیل شد (با پارامتر `?tab=` و بارگذاری شرطی داده هر تب): خلاصه، گفتگوها، کانال‌ها، دانش، محصولات و سفارش‌ها. تب خلاصه علاوه بر مسیر کاربر، پنل «آمار کسب‌وکار» را با شمارنده‌های مشتریان (کل + فعال ۳۰ روز)، گفتگوها، سفارش‌ها، محصولات، ایجنت‌ها و اتصال کانال فعال نسبت به سقف پلن نشان می‌دهد و «آخرین گفتگوها» اکنون کلیک‌پذیر و به متن کامل گفتگو متصل است.
- تب گفتگوها: ۳۰ گفتگوی آخر با مخاطب، کانال، ایجنت، وضعیت و تعداد پیام؛ هر ردیف (دسکتاپ) و هر کارت (موبایل) به `/admin/conversations/[id]` متصل است. تب کانال‌ها: نوع، ایجنت، فعال/غیرفعال، سلامت (ok/degraded/down/unknown)، آخرین پیام دریافتی و آخرین بررسی سلامت. تب دانش: منابع با نوع، وضعیت، تعداد قطعه و آخرین به‌روزرسانی. تب محصولات: قیمت (تومان)، موجودی، دفعات نمایش AI و وضعیت. تب سفارش‌ها: سفارش‌های فروشگاه با شماره، مشتری، اقلام، مبلغ (تومان)، وضعیت وووکامرسی و تاریخ.
- تب‌بند به‌صورت نوار اسکرول‌ افقی روی موبایل و همه تب‌ها الگوی «کارت موبایل + جدول مخفی دسکتاپ» را رعایت می‌کنند.
- لیست کاربران: ستون تکراری «کسب‌وکار» حذف شد (هر کاربر دقیقاً یک ورک‌اسپیس ۱:۱ با نام یکسان دارد) و جای آن ستون «وضعیت» نشست؛ KPI «کسب‌وکارها» با «مشتریان ثبت‌شده» (مجموع مخاطبین) جایگزین شد. کارت‌های موبایل و شیت جزئیات نیز از نام تکراری کسب‌وکار و لینک `/admin/workspaces` پاک شدند و شمار محصول به آن‌ها اضافه شد.
- تومان‌سازی: کارت «هزینه ۳۰ روز» در جزئیات ایجنت (قبلاً ریال خام)، نرخ دلار صفحه درآمد، زیرنویس صفحه مصرف AI، و فرم «تعرفه و پلن‌ها» که اکنون ورودی‌های ریالی (قیمت ماهانه، اعتبار هدیه، تعرفه پاسخ، نرخ دلار) را به تومان نمایش/وارد می‌کند و با ضریب ۱۰ در سیستم ذخیره می‌نماید (TOMAN_SCALE). مبالغ فروشگاه (محصول/سفارش) از قبل IRT/تومان هستند و بدون تبدیل نمایش داده می‌شوند.
- تست رگرسیون `tests/admin-user-panels.test.ts` (۱۲ تست) اضافه شد؛ کل مجموعه ۶۷۲ تست سبز، TypeScript بدون خطا، build پروداکشن موفق، ۱۶ اسکرین‌شات دسکتاپ/موبایل از صفحات واقعی تأیید شد.


## پنل ادمین — حذف آلارم قطع کانال، گزارش ورود و پیامک‌ها، صندوق گفتگوها (v3.6) — ۷ سپتامبر ۲۰۲۶

### حذف آلارم قطعی کانال
- پرونده کاربر (`app/admin/(dash)/users/[userId]/page.tsx`): نمایش «سلامت کانال» به‌طور کامل حذف شد — ثابت `HEALTH_STATUS` (سالم/ضعیف/**قطع**/بررسی‌نشده)، فیلدهای `healthStatus/healthCheckedAt/healthError` از کوئری، کارت آماری «سلامت کانال»، ستون «سلامت» جدول دسکتاپ و بج سلامت کارت موبایل. سیستم بررسی سلامت خود کانال (`lib/channels/health.ts` و پنل کاربر) دست‌نخورده باقی مانده است؛ فقط نمایش و آلارم آن در پنل ادمین حذف شد.
- جای کارت «سلامت کانال»، آمار مفید و غیرآلارمی «ورودی در ۷ روز اخیر» (کانال‌هایی که طی هفته گذشته پیام دریافت کرده‌اند) نشست.

### صندوق «آخرین گفتگوها» به سبک پنل کاربر (حذف تب گفتگوها)
- تب «گفتگوها» حذف شد؛ تب‌های پرونده اکنون ۵ مورد است: خلاصه، کانال‌ها، دانش، محصولات، سفارش‌ها. لینک‌های قدیمی `?tab=conversations` بدون خطا به تب خلاصه برمی‌گردند.
- بخش «آخرین گفتگوها» در تب خلاصه مطابق تب گفتگوهای پنل داشبورد کاربر بازطراحی شد (همان کامپوننت‌های production): آواتار مخاطب (با فال‌بک تصویر کانال)، نام نمایشی + هندل `@` اختصاصی هر کانال، پیش‌نمایش متن آخرین پیام (پاک‌سازی توکن محصول + نشان ↩ برای پاسخ ایجنت)، بج وضعیت گفتگو (باز/بسته/تحویل اپراتور با هایلایت توجه)، بج کانال، شمار پیام و زمان نسبی («۵ دقیقه پیش»). ۱۰ گفتگوی آخر نمایش داده می‌شود و ردیف‌ها مستقیم به متن کامل گفتگو (`/admin/conversations/[id]`) می‌روند.
- نسخه موبایل: کارت‌های لمسی با همان اطلاعات (آواتار، نام، پیش‌نمایش پیام، بج‌ها) — بدون جدول.
- لینک «همه گفتگوهای این کاربر» به لیست کامل `/admin/conversations?q=<تلفن کاربر>` (فیلتر تلفنی) متصل شد.

### گزارش ورود به پنل (آخرین ورود + تاریخچه)
- مدل جدید `LoginEvent` (id/userId/workspaceId/ip/userAgent/isNewUser/createdAt) + فیلد `lastLoginAt` روی `User` در `prisma/schema.prisma`؛ مایگریشن `20260907090000_login_events_sms_delivery` روی پروداکشن اجرا شد.
- `auth.ts`: هر ورود موفق با OTP یک رکورد LoginEvent (با IP و User-Agent واقعی درخواست) ثبت می‌کند و `lastLoginAt` را به‌روز می‌کند؛ خطای ثبت هرگز جریان ورود را نمی‌شکند (try/catch + captureWarning).
- تاریخچه گذشته از `OTPLog` رکوردهای تأییدشده بازسازی (backfill) شد: ۳۷ رخداد ورود، ۲۹ کاربر با آخرین ورود.
- UI: ردیف «آخرین ورود به پنل» در اطلاعات کاربر (زمان + نسبی)، پنل «گزارش ورود به پنل» با ۱۲ ورود آخر (نوع: ثبت‌نام/اولین ورود یا ورود موفق، دستگاه و مرورگر تشخیص‌داده‌شده از User-Agent، IP، زمان)، و شمار کل ورودها در خلاصه فعالیت.

### پیامک‌های ارسال‌شده به شماره کاربر
- مدل جدید `SmsDelivery` (phone/workspaceId/kind/status/ip/createdAt) — لاگ تحویل همه پیامک‌های سیستم به هر شماره.
- `lib/sms/ippanel.ts`: تابع `recordSmsDelivery` در همه مسیرهای ارسال (OTP موفق/ناموفق + همه الگوهای متنی: خوش‌آمد، خرید اشتراک، انقضای اشتراک، پایان آزمایشی، یادآوری راه‌اندازی، تکمیل راه‌اندازی) رکورد ثبت می‌کند؛ best-effort و غیرمسدودکننده.
- نسخه worker (`releases/automation-only-85313e679`) هم با همین لاگ پچ و ری‌استارت شد تا پیامک‌های زمان‌بندی‌شده (یادآوری‌ها) نیز ثبت شوند — تأیید عملکرد: رکوردهای ACTIVATION_REMINDER و SUBSCRIPTION_EXPIRING پس از دیپلوی ثبت شده‌اند.
- تاریخچه OTP از `OTPLog` بازسازی شد (۵۶ رکورد). تست زنده: ارسال واقعی کد ورود، رکورد SENT با IP در جدول ثبت کرد.
- UI: پنل «پیامک‌های ارسال‌شده به این شماره» با نوع هر پیامک (بج رنگی)، وضعیت ارسال/ناموفق، IP (برای OTP) و زمان + شمار کل در خلاصه فعالیت.

### تست و دیپلوی
- `tests/admin-user-panels.test.ts` بازنویسی شد (۱۵ تست: ۵ تب، حذف آلارم، صندوق، ورود، پیامک، موبایل، تومان) + تست‌های جدید tracking. کل مجموعه: ۶۷۸ تست سبز. TypeScript بدون خطا، build پروداکشن موفق، `prisma migrate deploy` اجرا شد، vignet-web و vignet-worker ری‌استارت شدند.
- اسموک‌تست HTTP با کوکی ادمین: ۱۵ صفحه ۲۰۰ + محتوای درست (شامل fallback تب حذف‌شده و صفحه گفتگوی فیلترشده). تست بصری Playwright دسکتاپ ۱۴۴۰×۹۰۰ و موبایل ۳۹۰×۸۴۴: ۱۵ اسکرین‌شات + بازبینی مدل بینایی — بدون همپوشانی یا عنصر شکسته.

## اسکلتون شیمردار همه‌جا + تحلیل و بهبود تازه — (v3.7) — ۹ سپتامبر ۲۰۲۶

### اسکلتون لودینگ موبایل‌سبک (Skeleton Screen + Shimmer) در کل محصول
- الگوی **Skeleton Screen با شیمر موجی (stagger)** — همان سبک کارت لودینگ نسخه موبایل — برای کل رابط کاربری: تب «تحلیل و بهبود» (۵ نقطه لودینگ: هدر متا، داده اولیه، جستجوی گفتگوها، سابقه/پیشنهادها، نتایج بررسی)، تمام صفحات پنل ادمین (/admin، ai، agents، users، conversations) و تمام صفحات داشبورد کاربر (overview، conversations، agents، contacts، analytics، appointments، billing، products).
- components/ui/skeleton.tsx: پریمیتیو Skeleton + کارت‌های آماده (پیشنهاد، گفتگو، سابقه، بررسی)؛ app/admin/(dash)/admin-skeletons.tsx و components/dashboard/dashboard-skeletons.tsx: بلوک‌های ساختاری هر پنل (هدر، آمار، نمودار، پنل، جدول، کارت ایجنت/مدل/مشتری، صندوق گفتگوها).
- شیمر با background-position است (RTL/LTR یکسان)، با prefers-reduced-motion خاموش می‌شود و از طریق loading.tsx مسیرها به‌صورت Suspense استریم می‌شود — صفحه شکل خودش را فوراً می‌کشد نه صفحه سفید.
- صندوق گفتگوهای داشبورد کاربر روی موبایل دقیقاً همان ConversationCardSkeleton تب تحلیل و بهبود را می‌بیند؛ products/loading.tsx از animate-pulse قدیمی به شیمر مشترک مهاجرت کرد.
- اعتبارسنجی: HTML استریم‌شده هر صفحه ۱۰۲ تا ۲۴۰ المان skeleton دارد؛ تست E2E با شبکه کند (CDP) دسکتاپ ۱۴۴۰px و موبایل ۳۹۰px کپچر و تأیید مدل بینایی شد (بدون سرریز/شکستگی).

### تحلیل و بهبود — تازگی بررسی، ترجیحات مشتری، فید فعالیت
- lastImprovementReviewAt روی گفتگوها (بازسازی از بررسی‌های DONE + ایندکس) — تحلیل‌ها گفتگوهای بررسی‌شده را رد می‌کنند و قدیمی‌ترین‌های بررسی‌نشده انتخاب می‌شوند (مایگریشن 20260909120000_improvement_review_freshness).
- ترجیحات تعامل هر مشتری در متادیتای CRM، تزریق به پرامپت با تبعیت از قوانین ایمنی؛ API فعالیت عمومی + نشانگر پیشرفت در هدر داشبورد (ImprovementActivityIndicator).
- قیمت‌گذاری هر درخواست AI (پاسخ/تحلیل/تست) با نرخ مدل انتخابی در lib/improvement/pricing.ts + بازگشت وجه رزرو ناموفق؛ تمام متون مارکتینگ/صورتحساب/مستندات fa-en به‌روز شد.

### دیت‌پیکر یکپارچه شمسی/میلادی
- جایگزینی دیت‌پیکر اختصاصی jalaali-js با react-day-picker یکپارچه (یک چیدمان، یک مدل کیبورد/a11y، ارقام و نام ماه locale-صحیح)؛ مصرف‌کنندگان: سفارش‌ها، نوبت‌ها، راه‌اندازی کانال اپراتور.

### رفع‌های عملیاتی
- پیام‌های فقط-مدیا به‌جای پاسخ ثابت «نمی‌توانم ببینم» به هندافف اپراتورِ بادوام و رایگان تبدیل شد (راستی‌آزمایی URL پیوست در همه کانال‌ها ممکن نیست).
- انکورهای پایدار پیام گفتگوها برای لینک‌های عمیق (scroll-mt زیر هدر چسبان).
...
## فاز A15 — ریشه‌یابی و رفع باگ آپلود عکس در سناریوهای اینستاگرام — ۲۱ سپتامبر ۲۰۲۶

### علامت بالینی (روی پروڈ)
- اپراتور در ادیتور سناریو عکس آپلود می‌کرد؛ گاهی آپلود کامل نمی‌شد و فرم، URL موقت مرورگر (`blob:`) را به‌جای URL واقعی در `messages[].mediaUrl` ذخیره می‌کرد.
- نتیجه: پیش‌نمایش عکس در داشبورد ۴۰۴/شکسته دیده می‌شد و اتوماسیون هرگز عکس را به اینستاگرام ارسال نمی‌کرد (کراولر Meta به blob: دسترسی ندارد) — فقط متن می‌رسید.
- رکورد اثبات‌شده: سناریوی «پاسخ به قیمت شلوار» (۲۰ سپتامبر) با `mediaUrl = blob:https://vigent.ir/ac99365c-…`.
- باگ دوم مرتبط: حذف عکسِ موجود در ادیتور، فایل را بلافاصله از دیسک DELETE می‌کرد؛ اگر اپراتور ذخیره را لغو می‌کرد، سناریو به فایل حذف‌شده اشاره می‌کرد (۴۰۴ دائمی). دو فایل سناریوی «سویشرت» (۵ و ۶ سپتامبر) به همین دلیل از دیسک حذف شده بودند.

### فایل‌های تغییر یافته

#### `components/instagram/automation-form.tsx`
- گارد سخت در `submit()`: اگر پیامی هنوز `blob:` در `mediaUrl` داشته باشد (آپلود در جریان یا ناموفق)، ذخیره بلاک می‌شود با پیام فارسی روشن («تا پایان آپلود صبر کنید / تلاش دوباره / حذف فایل»).
- دفاع دوم در `buildPayload()`: تحت هیچ شرایطی `blob:` در payload سریالایز نمی‌شود.

#### `components/instagram/media-uploader.tsx`
- `remove()` فقط برای فایل‌های آپلودشده در همین نشست (`file !== null`) فراخوانی DELETE می‌کند. آیتم‌های بازسازی‌شده از URL ذخیره‌شده (`initial`) دیگر فایل سرور را حذف نمی‌کنند — لغو ویرایش دیگر سناریو را خراب نمی‌کند. فایل‌های یتیم بی‌ضررند؛ فایل حذف‌شده-but-referenced نه.

#### `app/api/agents/[agentId]/instagram/automations/route.ts` + `[id]/route.ts`
- گارد Zod مشترک (`mediaUrlSchema`): mediaUrl باید خالی یا URL مطلق http(s) باشد؛ `blob:`/`data:` با خطای `MEDIA_URL_INVALID` رد می‌شود (POST و PATCH).

#### `lib/instagram/automation.ts`
- `isUnfetchableMediaUrl()`: در سه مسیر تحویل (گیت SOFT مستقیم، تحویل follow-gate، و legacy `action.mediaUrl`) رسانه با URL غیرقابل‌واکشی skip و با `captureWarning('instagram:automation:media-url-unfetchable')` ثبت می‌شود تا رکوردهای قدیمیِ خراب بی‌صدا Graph API را کالretry نکنند. مسیر اصلی `sendRichEntry` از قبل با `assertPublicHttps` محافظت بود.

#### `tests/instagram-automations-media-url.test.ts` (جدید)
- ۸ تست رگرسیون روی قرارداد API: رد blob:/data: در POST و PATCH (messages و action.mediaUrl)، پذیرش https معتبر و رشته خالی.

### تعمیر داده پروڈ (یک‌باره، ۲۰ سپتامبر ~۲۴:۰۰)
- `InstagramAutomation` «پاسخ به قیمت شلوار»: `messages[1].mediaUrl` از blob: به URL واقعی فایل آپلودی ۲۲:۳۲ همان روز جایگزین شد (فایل روی دیسک موجود و محتوای آن با VLM تأیید شد — عکس محصول شلوار نیم بگ CEEPORT مطابق متن سناریو). شمار blob: باقی‌مانده در جدول: صفر.

### اعتبارسنجی
- `tsc --noEmit` بدون خطا؛ ۱۱۹۶/۱۲۰۴ تست پاس (تنها fail: `marketing-homepage-ux.test.ts` — مربوط به تغییرات uncommitted صفحات marketing توسط توسعه‌دهنده دیگر، خارج از این فاز).
- دیپلوی با `deploy/manual-deploy-incremental.sh` (بیلد جدید در `.next-builds/`، ری‌استارت PM2، health 200).

## ۳۰ شهریور ۱۴۰۵ (۲۱ سپتامبر) — بازگرداندن glow و border-beam به پیش‌نمایش داشبورد صفحه اصلی (سبک writora.xyz)

طبق درخواست صریح کاربر، دو افکتی که در فاز قبل حذف شده بودند **دقیقاً مشابه writora.xyz** بازگردانده شدند؛ انیمیشن لود و بهینه‌سازی تصویر هم اضافه شد.

### `components/marketing/dashboard-showcase.tsx` (بازنویسی)
- **glow بنفش بالای عکس**: کلاس `marketing-showcase-gradient` (conic-gradient آبی-بنفش writora) + `animate-image-glow blur-[5rem]` — keyframes و مقدارها کپی verbatim از CSS بیلدشده writora.xyz (opacity 0 → 0.5 در 10٪ → 0.7، مدت 4s، تأخیر 0.6s، forwards).
- **beam چرخان دور قاب**: markup کامل beam کپی verbatim از writora.xyz (متغیرها: `--size:250; --duration:12; --anchor:90; --border-width:1.5; --color-from:#ffaa40; --color-to:#9c40ff; --delay:-9s`؛ ماسک دوبلایر padding-box/border-box با `mask-composite:intersect`؛ `::after` با `offset-path:rect(0 auto auto 0 round 250px)` و انیمیشن `border-beam 12s infinite linear`).
- **انیمیشن لود (blur-up)**: LQIP اینلاین ~320 بایت (JPEG بیست‌پیکسلی از همان اسکرین‌شات، blur-2xl + scale-110) که با اولین بایت HTML می‌آید؛ تصویر کامل پس از لود طی 720ms از `blur(22px)+scale(1.02)+opacity:0` به حالت شارپ transition می‌شود. گارد `img.complete` برای تصاویر کش‌شده + failsafe تایمری 5 ثانیه‌ای تا تصویر هرگز مخفی نماند.
- **WebP**: `public/assets/dashboard-preview.webp` (q92 با sharp) = 70KB در برابر PNG 451KB (۸۴٪ سبک‌تر) از طریق `<picture>` با fallback به همان PNG اصلی. `fetchpriority=high` + ابعاد صریح 1280×720 (بدون layout shift).
- کامپوننت client شد (state لود تصویر)؛ next/image با `<picture>` ساده جایگزین شد چون در حالت unoptimized چیزی اضافه نمی‌کرد.
- فیدهای پایین (دو لایه from-white)، قاب روشن و `max-w-[1200px]` دست‌نخورده ماندند.

### `app/globals.css` (افزودن بلوک به انتهای فایل)
- `.marketing-showcase-gradient` (conic)، `@keyframes image-glow` + `.animate-image-glow`، `@keyframes border-beam` + `.after\:animate-border-beam:after`، `.marketing-showcase-img` (blur-up) — همه verbatim از writora.xyz.
- اورراید کامل `prefers-reduced-motion: reduce`: توقف glow (opacity ثابت 0.7)، حذف beam، نمایش فوری تصویر بدون transition.

### اعتبارسنجی
- `tsc --noEmit` بدون خطا؛ دیپلوی با `deploy/manual-deploy-incremental.sh` → `DEPLOY_OK`، health 200.
- تأیید زنده با مرورگر headless: glow به opacity 0.699 رسید، beam با `border-beam 12s` فعال، `currentSrc=dashboard-preview.webp`، کلاس `is-loaded` اعمال شد، reveal فعال.
- VLM روی اسکرین‌شات دسکتاپ و موبایل: glow بنفش ✓ beam نارنجی→بنفش روی قاب ✓ تصویر شارپ ✓ فید پایین ✓ بدون glitch (مقایسه شد با اسکرین‌شات writora).
- همه مسیرها 200: `/`، `/en`، `/pricing`، `/blog`، `/solutions`، `/login`، `/status`.
- بکاپ فایل‌های قبل از این تغییر: `/root/vigent-manual-edit-backup-1789942022`.

## ۳۰ شهریور ۱۴۰۵ (۲۱ سپتامبر، بخش دوم) — اصلاح نمایش داشبورد، افکت beam سکشن پایانی، سکشن «اعتماد بهترین‌ها»، حذف سکشن «نحوهٔ کار» و فوتر جمع‌وجور

### `components/marketing/dashboard-showcase.tsx`
- فاصله داشبورد از متن هیرو در دسکتاپ کم شد (`sm:mt-20 lg:mt-24` → `sm:mt-16 lg:mt-16`) تا تصویر بالاتر بنشیند و با بزرگ‌ترشدن صفحه پایین نرود؛ موبایل دست‌نخورده (`mt-14`) طبق تأیید کاربر. خود تصویر همان اسکرین‌شات واقعی ماند.
- `--delay` مدارِ beam از `-9s` به `-2.4s` تغییر کرد: در لحظهٔ لود، beam روی **لبهٔ بالایی** قاب شروع می‌شود (پیش‌تر سمت چپ بود). اندازه‌گیری زنده: در t=0 روی ۲۰٪ محیط = وسط لبهٔ بالا؛ مسیر ساعتگرد ادامه می‌یابد.

### `components/marketing/future-cta.tsx` (سکشن «یک قدم جلوتر از هر پیام مشتری باشید»)
- افکت ناقص قبلی (`marketing-future-beam__*` با بال‌های کوچک) کامل حذف و با **beam نور writora.xyz** («Step into the Future of AI Content Creation») جایگزین شد — کپی verbatim: دو wing با conic-gradient بنفش (`from 70deg/290deg at center top`) + دو ماسک سفید در هر wing + نوار blur (8rem) + نوار شیشه‌ای backdrop-blur + orb بنفش + خط مویی `bg-purple-400` + کاور سفید بالای منشأ نور. تنها تغییر: `bg-background` → `bg-white` (زمینه روشن ویجنت).
- کارت سکشن `min-h-[42rem] sm:min-h-[46rem]` شد تا ترکیب‌بندی تمام‌صفحه‌ای مشابه writora شود؛ `data-scroll-reveal="scale"` برای ورود نرم.

### `app/globals.css`
- بلوک‌های قدیمی `.marketing-future-beam*` (اصلی + موبایل + سه keyframes ورود) حذف شدند (۵۷۵۷ کاراکتر؛ آکولادها بالانس). `.marketing-future-cta::after` و `.marketing-future-title` حفظ شدند.

### سکشن «نحوهٔ کار ویجنت» — حذف کامل
- `OperationsSection` + `ProductStage` + `ConnectedChannelLogos` و تمام کپی/سناریوهای پنج‌صنعتی از `components/marketing/hero.tsx` حذف شدند (۴۵۹ → ۱۳۳ خط؛ باندل کلاینت سبک‌تر شد).
- از render صفحه اصلی و import ها حذف شد. فایل‌های `social-proof.tsx` (نوار آمار زنده) و `lib/marketing/platform-stats.ts` نیز حذف شدند.

### سکشن «اعتماد بهترین‌های صنعت» (جایگزین نوار آمار)
- **`components/marketing/trusted-by-section.tsx` (جدید)**: همان سکشن Trusted by در writora.xyz (تیتر کوچک uppercase خاکستری + ردیف لوگوها با `flex-wrap` و `w-28`)؛ برای تم روشن: لوگوها `grayscale opacity-60` با hover رنگی. تا اولین لوگوی فعال ثبت نشود، سکشن رندر نمی‌شود.
- **پنل ادمین**: صفحه `/admin/showcase` به «محتوای صفحه اصلی» تبدیل شد و بلوک **«لوگوهای اعتماد»** را همین‌جا مدیریت می‌کند (افزودن/ویرایش/حذف/روشن‌خاموش/ترتیب + آپلود تصویر با پیش‌نمایش).
- **مدل داده**: `TrustedLogo` در Prisma (name، imageUrl، url، active، sortOrder) + migration `20260921120000_add_trusted_logo` (deploy شد).
- **API**: `/api/admin/trusted-logo` (GET/POST)، `/api/admin/trusted-logo/[id]` (PATCH/DELETE)، `/api/admin/trusted-logo/upload` (آپلود با همان گاردهای امضای تصویر showcase) و `/api/uploads/trusted-logo/[file]` (سرو با کش immutable) — همه پشت `isAdminAuthed` (تست شد: 401 بدون لاگین).

### فوتر و منو
- **فوتر دسکتاپ جمع‌وجور شد**: ستون «راهکارها» با ۷ لینک حذف شد؛ ستون‌ها از ۳ به ۲ («محصول» ۴ لینک + «یادگیری» ۳ لینک). لینک «اتصال‌ها» به `#unified-system` ریدایرکت شد (قبلاً به `#product` حذف‌شده اشاره می‌کرد).
- **منوی دسکتاپ**: «راهکارها» بین «صفحه اصلی» و «مستندات» اضافه شد (`/solutions`) با حالت active مسیرمحور.

### تست‌ها و دیپلوی
- `tests/marketing-homepage-ux.test.ts` با قرارداد جدید همگام شد: حذف ادعاهای ConnectedChannelLogos/`href="#solutions"`/social-proof، افزودن ادعاهای OperationsSection-غائب و TrustedBySection-حاضر. **۱۵/۱۵ تست پاس** (یک fail قدیمی هم رفع شد).
- `tsc --noEmit` بدون خطا؛ دیپلوی با `deploy/manual-deploy-incremental.sh` → `DEPLOY_OK`، health 200.
- تأیید بصری (اسکرین‌شات دسکتاپ + موبایل + VLM): beam داشبورد در لود بالای قاب ✓ glow ✓ سکشن beam پایانی با خط نازک درخشان + هاله بنفش روی عنوان ✓ (مشابه writora) ✓ رندر لوگوها (۳ لوگوی تست، سپس حذف) ✓ فوتر ۲ ستونه بدون ستون راهکارها ✓ موبایل بدون سرریز ✓
- همه مسیرها 200: `/`، `/en`، `/pricing`، `/blog`، `/solutions`، `/solutions/instagram`، `/login`، `/status`، `/docs`.
- بکاپ این فاز: `/root/vigent-manual-edit-backup-1789944429`

## ۱۴۰۵-۰۷-۳۰ — اصلاح هالهٔ بنفش، حذف افکت موس و بازنویسی افکت چراغ CTA پایانی (writora)

- **هالهٔ بنفش داشبورد:** لایه‌بندی writora بازتولید شد — هاله اکنون با `z-20` روی بالای اسکرین‌شات نقش می‌بندد (قبلاً پشت تصویر گیر کرده و فقط یک هالهٔ محو در فضای خالی دیده می‌شد). نوار هاله از ۲۴ پیکسل بالاتر از لبهٔ قاب شروع می‌شود و با blur پنج‌رم روی ~۴۰٪ بالای تصویر می‌نشیند؛ دقیقاً مثل writora. پرتordre مرزی (beam) با `z-30` بالای هاله قرار گرفت (ترتیب لایه‌های writora).
- **موقعیت پیش‌فرض تصویر:** فاصلهٔ دسکتاپ از ۶۴ به ۳۲ پیکسل کاهش یافت (تصویر ~۳۲ پیکسل بالاتر). موبایل دست‌نخورده ماند.
- **ترمیم ماسک beam:** پیشوند `!` اشتباهی روی کلاس خلاصهٔ `[mask:…]` باعث می‌شد ماسک شعاع مرزی غیرفعال شود (beam به‌صورت مربع خام می‌چرخید). به شکل اصلی writora برگردانده شد: `![mask-clip] ![mask-composite] [mask:…]` — مقادیر محاسبه‌شده اکنون `padding-box, border-box` و `intersect` است.
- **حذف افکت hover موس:** کامپوننت Spotlight (هالهٔ دنباله‌کنندهٔ نشانگر موس) از هیرو حذف شد؛ در صفحات راهکارها همچنان هست.
- **CTA پایانی:** انیمیشن «باز شدن» چراغ writora پیاده شد — دو بال مخروطی، گوی و خط نازک بنفش هنگام ورود کارت به دید، با تأخیر ۳۰۰ms و مدت ۸۰۰ms ease-in-out از ۱۵rem تا **عرض متن عنوان** باز می‌شوند (اندازه‌گیری زندهٔ `--lamp-open`). پیاده‌سازی CSS خالص روی state موجود `is-revealed` است تا framer-motion از مسیر اولیهٔ صفحه دور بماند (پیمانهٔ تست ۱۵/۱۵).
- **حذف سرتیتر:** برچسب «آیندهٔ ارتباط با مشتری، همین‌جاست» از بالای عنوان CTA حذف شد.
- تست‌ها: 15/15 سبز، tsc بدون خطا، همهٔ مسیرها ۲۰۰. پشتیبان: `/root/vigent-manual-edit-backup-1789948492`

## ۳۰ شهریور ۱۴۰۵ (بخش سوم) — هالهٔ بنفش پشت اسکرین‌شات داشبورد

### `components/marketing/dashboard-showcase.tsx`
- **لایه‌بندی هاله اصلاح شد (درخواست کاربر: «هاله باید زیر عکس باشه، الان رویش میفته»):** هالهٔ بنفش conic از `z-20` به `z-0` رفت و کانتینر عکسِ `overflow-hidden` صریحاً `z-10` گرفت. نتیجه: اسکرین‌شات داشبورد اکنون **روی** هاله نقاشی می‌شود و هاله فقط به‌صورت یک قوس نرم نور (حدود ۸۰px با احتساب پخش blur) **بالای لبهٔ بالایی قاب** دیده می‌شود — محتوای داشبورد (نمودارها، سایدبار، متن‌ها) دیگر هیچ شستشوی بنفشی رویش ندارد.
- هندسهٔ هاله دست‌نخورده ماند (شروع ~۲۴px بالاتر از لبهٔ بالایی قاب، `blur-[5rem]`، عرض `w-3/4`، انیمیشن fade-in همان `image-glow` با opacity نهایی ۰٫۷). ترتیب لایه‌های نهایی: هاله `z-0` < عکس `z-10` < beam مداری `z-30` < دیسالو پایینی `z-40/50`.
- کامنت‌های مستند (بالای فایل + JSX + بلوک CSS در `globals.css`) با توصیف «هاله پشت شات» همگام شد.

### تأیید زنده
- DOM: `glow z=0` (top 676) / `wrapper z=10` (top 700) / `beam z=30` ✓ — نوار بنفش پیکسلی دقیقاً ردیف‌های ۷۰–۱۴۹ و لبهٔ بالای عکس در ردیف ۱۵۰؛ **صفر پیکسل بنفش داخل ناحیهٔ اسکرین‌شات**.
- VLM دسکتاپ و موبایل: هاله پشت داشبورد، قوس نرم نور بالای لبهٔ قاب، محتوای داشبورد کاملاً خوانا بدون تینت بنفش، بدون گلیچ.
- تست‌ها: 15/15 سبز، `tsc --noEmit` بدون خطا، همهٔ مسیرها ( `/` `/en` `/pricing` `/blog` `/solutions` `/login` `/status` ) ۲۰۰. دیپلوی: `DEPLOY_OK dist=…-1789975240-1561326`. بکاپ نسخهٔ قبلی فایل: `dashboard-showcase.tsx.bak-v4-on-top` در همان پوشهٔ بکاپ.

## ۲ مهر ۱۴۰۵ (۲۳ سپتامبر) — چهار اصلاح صفحهٔ اصلی: تیترهای تک‌خطی، ترمیم سکشن قابلیت‌ها در موبایل و هالهٔ تلویزیونی CTA

### ۱ و ۳ — تیترهای تک‌خطی، تک‌رنگ و کوچک‌تر (هر سه سکشن)
- **کلاس جدید `.marketing-title-oneline` در globals.css**: `white-space: nowrap` + اندازهٔ فونت سیّال نسبت به عرض خودِ کانتینر (`cqw`) با حاشیهٔ امن ۲٪ و سقف `--title-max`؛ خط اول `font-size` به‌صورت fallback مبتنی بر `vw` برای موتورهای بدون واحد کانتینری. هدر هر سکشن `[container-type:inline-size]` گرفت.
- «از اولین پیام مشتری تا نتیجه‌ای که در کسب‌وکار ثبت می‌شود» (سکشن قابلیت‌ها): تقسیم titleStart/titleAccent و گرادیان بنفش→سرخ حذف شد؛ حالا یک خط، یک رنگ `#111118`. ضرایب fa/en از اندازه‌گیری canvas با فونت واقعی (IRANSansWeb ۶۰۰ / geistSans ۶۰۰): fa `4.2cqw` سقف `1.75rem`، en `2.74cqw` سقف `1.5rem`.
- «بعد از ثبت‌نام دقیقاً چه اتفاقی می‌افتد؟» (سکشن شروع): `MarketingSectionHeading` صریحاً `titleClassName`/`titleStyle` گرفت (پیش‌فرض قبل بدون تغییر برای بقیهٔ مصرف‌کننده‌ها)؛ fa `6.5cqw` / en `6.2cqw`، سقف `2.25rem`.
- **اندازه‌گیری تأییدشده (تک‌خط در همهٔ عرض‌ها)**: ۳۲۰px → فونت‌های ۱۱/۱۸/۲۰px ؛ ۳۹۰ → ۱۳.۹/۲۲.۸/۲۵.۹px ؛ ۷۶۸ → ۲۷.۸/۳۶/۴۱.۶px ؛ ۱۴۴۰ → ۲۸/۳۶/۴۱.۶px — همه `fits=true`، `lines=1`.

### ۲ — ترمیم سکشن «یک سیستم، نه چند ابزار پراکنده» در موبایل
- **ریشهٔ خرابی پیدا شد**: اورراید موبایل قدیمی (`opacity:1; transform:none` با اختصاصی‌بودن ۰٬۳٬۱) در ترتیب فایل با قواعد `html[dir='rtl'] … [data-scroll-reveal='side'/'-reverse']` (همان ۰٬۳٬۱ ولی جلوتر در سورس) می‌باخت؛ نتیجه: کارت‌های آشکار ولی شیفت‌خورده به اندازهٔ ±۱۶px کنار ریل، نقطه‌های خارج از خط ریل و ظاهر «نصفه/گیرکرده».
- **اصلاح**: بلوک جدید در `@media (max-width: 767px)` با سلکتور مرکب `.marketing-story-section.marketing-section-capabilities/-onboarding [data-scroll-reveal]` (۰٬۴٬۱ — همیشه بر قواعد rtl می‌برد): fade-up عمودیِ سریع ۳۰۰ms با stagger ۲۰ms و `is-revealed` → identity. تبدیل افقی در موبایل حذف شد تا نقطه‌ها حین انیمیشن هم روی ریل بمانند.
- **متن کارت‌ها دیگر بریده نمی‌شود**: `line-clamp-2` حذف شد (توضیح کامل در موبایل). **تگ‌ها در موبایل پیدا شدند** (قبلاً `hidden sm:flex`). **دو دایرهٔ blur بزرگ ۹۰/۱۱۰px زیر `lg` مخفی شدند** (سرعت پینت/اسکرول موبایل).
- تأیید زندهٔ ۳۹۰px: `railCenter=342 / dotCenter=340`، صفر کارت شیفت‌خورده، همهٔ لبه‌های راست کارت‌ها یکسان (۳۲۱)، `lineClamp=none`، ۳ تگ در هر کارت؛ VLM: «dots centered, no shifting, polished».

### ۴ — CTA پایانی: متن جدید + هالهٔ تلویزیونی مثل writora
- **متن کوتاه‌تر**: «یک قدم جلوتر از هر پیام مشتری باشید» → **«جلوتر از بقیه کسب‌وکارها باشید»** (en: «Stay ahead of every other business»)؛ همان درمان تک‌خطی/تک‌رنگ با fa `8cqw` / en `5.75cqw`، سقف `2.6rem`. گرادیان `.marketing-future-title` حذف شد.
- **هالهٔ بازتر و نرم‌تر (حالت تلویزیون)**: `--lamp-open` دیگر برابر عرض متن نیست؛ `clamp(متن+۱۲۸px، حداقل ۳۰rem، حداکثر ۴۶rem)` — بال‌ها/خط مویی/گوی فراتر از دو سر متن باز می‌شوند و نور از متن عبور می‌کند (مثل glow تلویزیون). موبایل: بال‌ها ۴۸۰px روی کارت ۳۶۴px = نور تمام‌عرض. زمان‌بندی writora (۳۰۰ms تأخیر / ۸۰۰ms ease-in-out) دست‌نخورده.
- **باگ ریگرسیون که همان جلسه گرفت و رفع شد**: wrapper محتوا آیتمِ flex با عرض shrink-to-fit بود و با فرزندان cqw به ۰ فرومی‌ریخت (تیتر صفر پیکسل در EN) → `w-full` گرفت.

### تست و دیپلوی
- 15/15 تست سبز، `tsc --noEmit` بدون خطا (هر دو دیپلوی)، دیپلوی‌ها: `…-1790154268-2787174` و `…-1790154623-2793484`، health 200، همهٔ مسیرها ۲۰۰.
- VLM: تیترهای تک‌خطی تک‌رنگ (دسکتاپ+موبایل)، هالهٔ CTA پهن‌تر از متن با لبهٔ نرم و بدون گلیچ، اسکرین‌شات تمام‌صفحهٔ موبایل CLEAN.
- بکاپ نسخهٔ قبل: `/root/vigent-manual-edit-backup-1789948492/*.bak-v5` (و `globals.css.bak-v6`).


## ۱۴۰۵/۰۷/۰۱ — نشست هفتم: تیتر CTA بالاتر، زیر نور چراغ

### ۱ — «جلوتر از بقیه کسب‌وکارها باشید» بالاتر رفت، داخل نور
- **مشکل**: کارتِ CTA پایانی هنوز ترکیب‌بندی writora را داشت — محتوا وسطِ کارتِ بلند (sm:min-h-[46rem]) وسط‌چین بود و تیتر ~۲۱۵px زیرِ خطِ نور شناور مانده بود؛ از نور «جدا» می‌افتاد و حال‌وهوای خودش را از دست می‌داد (بازخورد کاربر: متن را بالاتر بیاور که بهش حال بخورد).
- **راه‌حل**: ستون محتوا با یک `pb-32 sm:pb-40` روی همان کارتِ وسط‌چین به بالا متمایل شد (بی‌تغییر در جریان چیدمان، بدون transform) و کارت هم کمی جمع‌تر شد: `min-h-[42rem]→[39rem]` و `sm:min-h-[46rem]→[42rem]` تا پایین‌کارته فضای مرده نیفتد.
- **نتیجهٔ اندازه‌گیری زنده**: تیتر دسکتاپ از y=282 به **y=175** و موبایل از y=260 به **y=172** رسید — یعنی دقیقاً جایی که گویِ بنفش (`blur-2xl`) پشتِ متن شروع به محو شدن می‌کند؛ خط موییِ نور (y=68) حالا تاجِ تیتر است و نور از پشتِ متن عبور می‌کند (دسکتاپ و موبایل، fa و en).
- VLM: دسکتاپ «balanced & premium، متن کاملاً خوانا»؛ مقایسهٔ قبل/بعد موبایل: نسخهٔ جدید «significantly better — heading bathed in the light، ریتم عمودی نرم، بدون هیچ گلیچ».

### ۲ — ریزفیکس: اندازه‌گیری `--lamp-open` بعد از لود فونت
- اولین `measure()` ممکن بود روی فونتِ جایگزین (قبل از سواپ وب‌فونت) اجرا شود و عرض تیتر را کم‌تر بگیرد؛ در `/en` خطِ چراغ ۶۳۵px باز می‌شد در حالی که متن ۶۶۳px بود (نور از متن کوتاه‌تر می‌ماند).
- `document.fonts.ready` به همان effect اضافه شد تا بعد از آماده‌شدن فونت‌ها دوباره اندازه گرفته شود: en حالا **۷۳۴px در برابر متن ۶۶۳px** و fa **۶۳۳px در برابر ۵۰۵px** — نور مثل حالت تلویزیونیِ writora از دو سر متن فراتر باز می‌شود.

### تست و دیپلوی
- 15/15 تست سبز، `tsc --noEmit` بدون خطا (هر دو دیپلوی)، دیپلوی‌ها: `…-1790156895-2828696` و `…-1790157497-2832579`، health 200، همهٔ مسیرها ( / , /en , /pricing , /blog , /solutions , /login , /status ) ۲۰۰.
- بکاپ نسخهٔ قبل: `/root/vigent-manual-edit-backup-1789948492/future-cta.tsx.bak-v6-lowtitle`.
- اسکرین‌شات‌ها: `download/v7-desktop-title-in-light.png` و `download/v7-mobile-title-in-light.png`.

## ۱۴۰۵/۰۷/۰۲ — نشست هشتم: داشبورد ادمین — فعالیت واقعی، بازه‌های کاری و درآمد/موجودی

### ۱ — «کاربران فعال» بر اساس آخرین فعالیت (نه صرفاً تعداد مکالمه)
- **مشکل**: کاربری که همین الان فعالیت کرده بود، بالای لیست نمی‌آمد؛ مرتب‌سازی فقط با `COUNT` مکالمه بود.
- **رفع**: `topActiveUsers` در `lib/admin/charts.ts` بازنویسی شد — `lastActivityAt = GREATEST(آخرین مکالمه ورک‌اسپیس، آخرین ورود پنل کاربر)` و `ORDER BY lastActivityAt DESC` (تی‌بریک: تعداد مکالمه ۳۰ روز). `JOIN` به `LEFT JOIN` تغییر کرد تا کاربر با ورود اخیر ولی مکالمه‌ی صفر هم دیده شود؛ `HAVING` فعالیت را به پنجره ۳۰ روز محدود می‌کند. متن زیرعنوان و حالت خالی به‌روز شد.

### ۲ — دکمه‌های ۷ روز / ۳۰ روز / ماهانه واقعاً داده عوض می‌کنند
- **مشکل**: نمودار «گفتگوها» همیشه سری ۷روزه (`kpiTrends.conv`) را می‌گرفت؛ با انتخاب ۳۰ روز فقط عنوان عوض می‌شد و داده ثابت می‌ماند (کاربر آن را «خرابی دکمه‌ها» می‌دید). در حالت ماهانه هم نمودار گفتگوها اصلاً نمایش داده نمی‌شد.
- **رفع**: در `app/admin/(dash)/page.tsx` سری گفتگوها به `rangeSeriesPromise` منتقل شد (`conversationsDaily(days)` برای ۷/۳۰ روز). تابع جدید `conversationsMonthly(12)` در `lib/admin/charts.ts` (هم‌الگوی `revenueIRRMonthly`) و در حالت ماهانه دو نمودار کنار هم رندر می‌شوند: «درآمد ماهانه» + «گفتگوهای ماهانه».

### ۳ — «توزیع پلن‌ها» → «درآمد به تفکیک پلن»
- تابع جدید `revenueByPlan()` در `lib/admin/charts.ts`: جمع پرداخت‌های موفق `SUBSCRIPTION` به‌تفکیک پلن (IRR + USD×نرخ تبدیل پلتفرم)، با رعایت `excludeFromAdminReports`. `DonutChart` در `components/admin/trend-chart.tsx` پروپ اختیاری `format="irr"` گرفت (لجند و تولتیپ تومانی). وسط دونات: جمع کل تومان.

### ۴ — «هزینه OpenRouter» → «موجودی OpenRouter»
- کارت جدید از `getOpenRouterAccountUsage()` (از قبل موجود در `lib/admin/ai-usage.ts`) استفاده می‌کند: مانده اعتبار زنده از API اوپن‌روتر (`total_credits − total_usage`)، زیرنویس «مصرف‌شده X از Y اعتبار حساب»، رنگ کارت بر اساس آستانه (≤۱$ قرمز، ≤۵$ کهربایی). fallbackهای «کلید تنظیم نشده / دریافت ناموفق» با نمایش مصرف ۳۰ روز.

### تست و دیپلوی
- `tsc --noEmit` بدون خطا؛ `npm test`: ۱۲۰۶ پاس (۲ fail مربوط به `marketing-home-variants` است که تغییرات ۳ روز پیشِ session قبل روی فایل‌های marketing است و به این نشست ربطی ندارد).
